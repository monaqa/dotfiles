...
def "into-sk" [] {
    $in | str join (char nl) | sk
}

def --env "ghq cd" [] {
  let dir = ghq list -p | sk;
  cd $dir
}
...
def "history grep" [] {
    history | sort-by command | uniq-by command | sort-by index | get command | into-sk
}

def "gh pr-fuzzy" [] {
    let pr_num = gh pr list --json number,title --jq '.[] | [.number, .title] | @tsv' | sk | from tsv -n | get column1.0;
