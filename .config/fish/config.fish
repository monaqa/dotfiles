...
# thanks: https://gist.github.com/tommyip/cf9099fa6053e30247e5d0318de2fb9e
function __auto_setenv --on-variable PWD --description 環境変数を自動で設定
    status --is-command-substitution; and return
    ...
    if test -e "$cwd/.local_ignore_justfile"
        export JUST_JUSTFILE=".local_ignore_justfile"
    end
end
# }}}
...
function __just_list_fuzzy
    if set -l d (just --list --list-heading "" --list-prefix "" | $FUZZY_FINDER)
        echo "just $d"
    end
end
abbr -a ss --function __just_list_fuzzy
# }}}
